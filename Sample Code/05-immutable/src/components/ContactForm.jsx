...
export default class ContactForm extends React.Component {
  ...
  componentWillReceiveProps(newProps) {
    ...
    // If values given
    if (typeof newProps.values !== 'undefined') {
  ...
  getSubmitLabel() {
    return (typeof this.props.values === 'undefined' ? 'Add Contact' : 'Update Contact');
  }
        ...
        <div>
            <form className="form-inline row" onSubmit={this.onSubmit.bind(this)}>
              ...
              <div className="col-xs-6">
                <button type="submit" className="btn btn-primary pull-right">{this.getSubmitLabel()}</button>
              </div>
            </form>
        </div>
    );
